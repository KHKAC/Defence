...
public class EnemyManager : MonoBehaviour
{
    ...
    IEnumerator SpawnEnemy()
    {
        ...
        {
            int enemyIndex;
            if(currentWave.isStatic && currentWave.maxEnemyCount == currentWave.enemyPrefabs.Length)
            {
                // 고정인 경우 카운트가 곧 인덱스
            ...
            // 생성 시간 기다렸다가 다음 적 생성
            float waitTime;
            if(currentWave.isStatic && currentWave.maxEnemyCount == currentWave.spawnTimeStatic.Length)
            {
